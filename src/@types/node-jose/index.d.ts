...
declare module 'node-jose' {
  ...
  export module JWE {
    ...
    type decryptCritHeaderOptions = {
      handlers: {
        [crit_header: string]: boolean | Function | decryptCritProcessors
      }
    }
    ...
    function createDecrypt(
      ks: JWK.KeyStore | JWK.Key,
      opts?: decryptCritHeaderOptions
    ): Decrypter
    ...
    function createEncrypt(
      opts: encOpts,
      rcpts: any | JWK.Key | JWK.Key[] | { key: JWK.Key } | { key: JWK.Key }[]
    //    rcpts: JWK.Key | JWK.Key[] | { key: JWK.Key } | { key: JWK.Key }[]
    ): Encrypter
    function createEncrypt(
      rcpts: JWK.Key | JWK.Key[] | { key: JWK.Key } | { key: JWK.Key }[]
    ): Encrypter
  ...
  */
  ...
  export module JWS {
    ...
    type verifyOptions = {
      handlers: {
        [crit_header: string]: boolean | Function | verifyCritProcessors
      }
    }
    ...
    function createSign(
      opts: signOpts,
      signs: JWK.Key | JWK.Key[] | signSignatory[]
    ): Signer
  ...
  export module util {
     ...
     */
    function asBuffer(
      input: Buffer | string | Array<any> | ArrayBuffer | ArrayBufferView,
      encoding?: string
    ): Buffer
  ...
  type parseCompactReturn = {
    ...
    perform: (
      ks: JWK.KeyStore
    ) => Promise<JWE.decryptReturn> | Promise<JWS.verifyReturn>
  }
  ...
  export function parse(
    input: Buffer | string | object
  ): parseCompactReturn | parseJSONReturn
